...
// Update the representatives array to have just one representative for simplicity
...
const mockAppointments: Appointment[] = [
  ...
  },
  {
    ...
    appointmentType: "Account Review",
    startTime: "1:00PM",
    endTime: "2:00PM",
    representative: "Sarah Johnson",
    status: "scheduled" as const,
  ...
  },
  {
    ...
    customerEmail: "william.davis@example.com",
    appointmentType: "New Product Onboarding",
    startTime: "2:30PM",
    endTime: "3:30PM",
    representative: "Michael Rodriguez",
    status: "scheduled" as const,
...
export default function AppointmentScheduler() {
  ...
  const [newStartTime, setNewStartTime] = useState("8:00AM")
  const [newEndTime, setNewEndTime] = useState("9:00AM")
  const [newRepresentative, setNewRepresentative] = useState("John Carpenter")
  ...
  const toggleActionItem = (id: string) => {
    setActionItems(actionItems.map((item) => (item.id === id ? { ...item, checked: !item.checked } : item)))
  }
  ...
  const handleAddAppointment = () => {
    const newAppointment = {
      ...
      customerEmail: `${newGuest.toLowerCase().replace(" ", ".")}@example.com`,
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      {/* Update the calendar view section to fix the duplication issue */}
      {/* Replace the entire representatives mapping section with this: */}
      <div className="w-full flex-1 overflow-auto border rounded-md">
        <div className="flex flex-col">
          <div className="grid grid-cols-1 border-b bg-muted/30">
            {representatives.map((rep) => (
              <div key={rep.id} className="flex items-center gap-2 p-3">
                <div className="w-8 h-8 rounded-full overflow-hidden">
                  <img src={rep.avatar || "/placeholder.svg"} alt={rep.name} className="w-full h-full object-cover" />
                </div>
                <span className="font-medium">{rep.name}</span>
                <span className="text-xs text-muted-foreground">{rep.expertise.join(", ")}</span>
              </div>
            ))}
          </div>
          ...
          <div className="relative" style={{ height: "600px" }}>
            ...
            {appointments
              .filter((apt) => representatives.some((rep) => rep.name === apt.representative))
              .map((appointment) => {
        ...
        <DialogContent className="sm:max-w-[500px]">
          ...
          <div className="space-y-4 py-4">
            ...
            <Select defaultValue="john" onValueChange={setNewRepresentative}>
              ...
              <SelectContent>
                <SelectItem value="John Carpenter">John Carpenter (Loans, Mortgages)</SelectItem>
                <SelectItem value="Sarah Johnson">Sarah Johnson (Customer Support)</SelectItem>
                <SelectItem value="Michael Rodriguez">Michael Rodriguez (Sales, Onboarding)</SelectItem>
              </SelectContent>
            </Select>
          </div>
          <DialogFooter>
            ...
            <div className="space-y-4 py-2">
              ...
              <Tabs defaultValue="summary">
                    ...
                    {isEditingSummary ? (
                      <div className="space-y-3">
                        ...
                        <div>
                          ...
                          <Input value={sentiment} onChange={(e) => setSentiment(e.target.value)} className="text-sm" />
                        </div>
                      </div>
                    ) : (
                      <>
                ...
                <TabsContent value="checklist" className="space-y-4 pt-4">
                  ...
                  {!isAddingAction && (
                    <div className="pt-2">
                      <Button variant="outline" size="sm" className="gap-1" onClick={() => setIsAddingAction(true)}>
