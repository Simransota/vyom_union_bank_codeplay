...
def handle_db_query(session_id: str, user_input: str, query_details: Dict = None) -> str:
    ...
    """
    # Mock response - in real code this would query the actual database
    query_type = "transactions" if "transaction" in user_input.lower() else "general"
    
    if query_type == "transactions":
        return {
            "status": "success",
            "data": [
                {"date": "2025-03-10", "amount": -120.50, "description": "Grocery Store"},
                {"date": "2025-03-08", "amount": -45.00, "description": "Coffee Shop"},
                {"date": "2025-03-05", "amount": -250.75, "description": "Utility Bill"},
                {"date": "2025-03-01", "amount": 2500.00, "description": "Salary Deposit"},
                {"date": "2025-02-28", "amount": -85.20, "description": "Restaurant"}
            ],
            "balance": 4325.60
        }
...
def handle_service_request(session_id: str, user_input: str) -> str:
    ...
    if "credit card" in user_input.lower():
        service_type = "credit card application"
    ...
    return {
        ...
        "ticket_id": "SR" + str(hash(user_input + session_id))[-6:],
        "message": f"Your {service_type} request has been created and will be processed within 24 hours."
    }
...
def create_chat_session(groq_api_key, session_id=None):
    """
